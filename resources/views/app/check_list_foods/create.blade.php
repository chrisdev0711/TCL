...
<main class="-mt-28 pb-8">
    <section aria-labelledby="hire-contract">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:max-w-7xl lg:px-8">
            <x-slot name="title">
                <a href="{{ route('check-list-foods.index') }}" class="mr-4"
                    ><i class="mr-1 icon ion-md-arrow-back"></i
                ></a>
                @lang('crud.check_list_foods.create_title')
            </x-slot>
            <form
                method="POST"
                action="{{ route('check-list-foods.store') }}"
                role="form"
                class="mt-4"
                enctype="multipart/form-data"
                id="check-list-foods-form"
            >
                ...
                @include('app.check_list_foods.form-inputs')
                <div class="mt-10">
                    <a id="back_btn" class="button float-left mr-5">
                        <i class="mr-1 icon ion-md-return-left text-primary"></i>
                        @lang('crud.common.back')
                    </a>
                    <a id="abort_food_btn" class="bg-orange-500 hover:bg-orange-700 text-white py-1 px-4 rounded-md float-left ml-5 mr-5 cursor-pointer">
